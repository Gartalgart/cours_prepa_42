# Séquence 1 — Les bases du C

Cette séquence couvre les fondamentaux de la programmation en C : de la mise en place de l'environnement jusqu'aux pointeurs, en passant par les types, les opérateurs, les boucles et les fonctions.

## Supports de la séquence

| Support | Titre | Statut |
|---|---|---|
| [Introduction](../introduction/index.md) | Découverte du C | En cours |
| [Support 00](support-00/cours.md) | Préparer son environnement de travail | ✅ Disponible |
| [Support 01](support-01/cours.md) | Premier programme | ✅ Disponible |
| [Support 02](support-02/cours.md) | Caractères et affichage | ✅ Disponible |
| [Support 03](support-03/cours.md) | Variables et types | ✅ Disponible |
| Support 04 | Opérateurs | À venir |
| Support 05 | Conditions | À venir |
| Support 06 | Boucles | À venir |
| Support 07 | Fonctions | À venir |
| Support 08 | Tableaux et chaînes | À venir |
| Support 09 | Pointeurs (initiation) | À venir |
| Bilan | Mini-projet récapitulatif | À venir |

!!! note "À retenir"
    Chaque support inclut un cours et des exercices. Fais les exercices avant de passer au support suivant.

---

➡️ **Commencer par le Support 00 :** [Préparer son environnement](support-00/cours.md)
